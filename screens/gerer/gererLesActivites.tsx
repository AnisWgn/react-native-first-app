...
// Liste des activités (Firestore). Création admin uniquement ; suppression depuis le détail.
export default function GererLesActivites({ navigation }) {
  ...
  const loadActivites = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const ref = collection(db, 'activites');
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [navigation, loadActivites]);

  useFocusEffect(
    useCallback(() => {
      if (auth.currentUser) loadActivites();
    }, [loadActivites])
  );
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>
          Activités{isAdmin(role) ? ' (admin)' : ''}
        </Text>
        {isAdmin(role) && (
          <Pressable
            style={({ pressed }) => [styles.btnCreer, pressed && styles.btnPressed]}
            onPress={() => navigation.navigate('editActivite')}
          >
            <Text style={styles.btnCreerText}>Créer une activité</Text>
          </Pressable>
        )}
      ...
      {loading ? (
        ...
        <View style={styles.centerBox}>
          <Text style={styles.emptyText}>Aucune activité dans la collection</Text>
        </View>
      ) : (
        <FlatList
          style={styles.list}
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              <TouchableOpacity
                ...
                activeOpacity={0.7}
                onPress={() => navigation.navigate('detailActivite', { activite: item })}
              >
                <View style={styles.cardAccent} />
                <View style={styles.cardBody}>
                  <Text style={styles.cardTitle}>{item.libActivités ?? item.libActivité ?? item.nom ?? item.id}</Text>
